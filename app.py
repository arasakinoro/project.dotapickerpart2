...
def recommend_heroes(my_role, enemies_names, top_k=7, mode='average'):
    ...
    for enemy_name in enemies_names[:5]:
        ...
        matchups = get_matchups(enemy_id)
        if matchups:
            valid_enemies += 1
    ...
    if valid_enemies == 0:
        return ["Нет данных по врагам — подожди 5 минут или попробуй других врагов"] * top_k
